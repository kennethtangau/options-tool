...
def set_strategy(strategy_name):
    ...
    for t in legs:
        ...
        if t['type'] == 'Long Share':
            l['price'] = t.get('price', ref)
            l['share_val'] = l['price'] * l['quantity']
        else:
            l['strike'] = t['strike']
...
for i, leg in enumerate(st.session_state.legs):
    ...
    if leg['type'] == 'Long Share':
        q_key, v_key = f"qty_{lid}", f"val_{lid}"
        ...
        prem_unit_key, prem_total_key = f"prem_{lid}", f"prem_tot_{lid}"
        if 'total_prem' not in leg: leg['total_prem'] = leg['premium_per_opt'] * qty
        
        cols[4].number_input("Prem ($) per Opt", value=float(leg['premium_per_opt']), step=0.05, format="%.2f", 
